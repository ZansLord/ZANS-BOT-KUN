...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('ZansBotz Kun', {
		font: 'Standard',
...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	kurr.browserDescription = [ 'ZansBotz Kun', 'ubuntu', '3.0' ]

	kurr.on('qr', () => {
	console.log(color('[','white'), color('!','red'), color(']','white'), color('SCAN QR NYA TOLOL'))
})
	kurr.on('credentials-updated', () => {
		fs.writeFileSync('./session.json', JSON.stringify(kurr.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'ZansBotz Kun Loadming...')
	})
	fs.existsSync('./session.json') && kurr.loadAuthInfo('./session.json')
	kurr.on('connecting', () => {
		start('2', 'CONNECT BRAD')
	})
	kurr.on('open', () => {
		success('2', 'Meng Day Komnek')
	})
...
kurr.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "6285869074622@s.whatsapp.net"
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `ZansBotz-Kun`, img,but = [{buttonId:`donasi`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `ZansBotz Kun`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `SAYONARA !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
...
kurr.on('group-update', async (anu) => {
    ...
    kurr.on('chat-update', async (message) => {
        ...
        ownerNumber = ["6285869074622@s.whatsapp.net","6285869074622@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
